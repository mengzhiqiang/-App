...
- (void)_setupNavigationBarTitle
{
    ...
    ////设置联系人未标题
    [self.customNavBar setTitle:self.conversationModel.name];
}
...
{
    NSDictionary *diciton = aCell.model.emModel.ext ;
    if ([diciton objectForKey:@"videoID"]) {
        ///跳转到视频详情页
//        return;
    }
...
#pragma mark 构造拓展消息 转发视频
-(void)_forwardCustomVideoMsg:(EMMessage *)aMsg
                       toUser:(NSString *)aUsername{
    ...
    // 生成message
   NSDictionary *messageExt = @{@"videoID":@"100"};
