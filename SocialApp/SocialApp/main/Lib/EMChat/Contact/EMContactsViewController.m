...
//    }];
    ...
    self.notifCell.avatarView.image = [UIImage imageNamed:@"iocn_xpy"];
    self.notifCell.nameLabel.text = @"新朋友";
    ...
    [self.notifCell.avatarView mas_updateConstraints:^(MASConstraintMaker *make) {
              make.top.equalTo(@15);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSInteger row = indexPath.row;
    if (section == 0 && row == 0) {//申请cell特殊化，需要显示角标
    ...
    if (section == 0) {
        if (row == 0) {
            cell.avatarView.image = [UIImage imageNamed:@"btn_newfriend"];
            cell.nameLabel.text = @"新朋友";
        } else if (row == 1) {
            cell.avatarView.image = [UIImage imageNamed:@"btn_tjql"];
            cell.nameLabel.text = @"添加群组";
        }
        else if (row == 2) {
            cell.avatarView.image = [UIImage imageNamed:@"btn_saoyisao"];
            cell.nameLabel.text = @"扫一扫";
        }
        [cell.avatarView mas_updateConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(@15);
            make.left.equalTo(@15);
            make.width.equalTo(@30);
            make.bottom.equalTo(@-15);
