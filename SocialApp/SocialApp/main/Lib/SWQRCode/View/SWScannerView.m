...
#define Scanner_Width (self.frame.size.width-(48*2)) /** 扫描器宽度 */
...
NSString *const ScannerLineAnmationKey = @"ScannerLineAnmati0nKey"; /** 扫描线条动画Key值 */
...
//    borderPath.lineCapStyle = kCGLineJoinRound;
    ...
    for (int index = 0; index < 4; ++index) {
        ...
        tempPath.lineCapStyle = kCGLineJoinRound; //线条拐角
        tempPath.lineJoinStyle = kCGLineCapRound; //终点处理
        UIColor *strokeColor = [UIColor whiteColor];
        [strokeColor set];
        
        //        self.config.scannerCornerColor = [UIColor whiteColor];
