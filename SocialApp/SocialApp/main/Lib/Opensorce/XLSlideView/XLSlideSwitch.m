...
//button间隔
static const CGFloat ButtonMargin = 5.0f;
//button标题正常大小
...
-(void)buildUI
{
    ...
    _topScrollView.backgroundColor =  rgba(208, 172, 129, 1);
    ...
    _lineView.frame = CGRectMake(0, _topScrollView.bounds.size.height, self.bounds.size.width, 2);
    ...
    _shadowImageView.layer.cornerRadius = 1.5;
    self.lineWidth = 53;
    self.ButtonFontSelectedSize = 14;
    
}
-(void)setAdjustBtn:(BOOL)adjustBtn
{
...
-(void)updateButtons
{
    ...
    for (NSInteger i = 0; i<_buttons.count; i++) {
        ...
        CGSize textSize = CGSizeMake(70, 40);
        ...
        if (i == _selectedIndex) {
            button.titleLabel.font = [UIFont fontWithName:@"Helvetica-Bold" size:_ButtonFontSelectedSize];
            button.selected = true;
        }else{
            button.titleLabel.font = [UIFont fontWithName:@"Helvetica-Bold" size:ButtonFontNormalSize];
...
//            [button setFrame:CGRectMake(i*btnWidth + margin, 0, btnWidth, TopScrollViewHeight)];
            float btnWidth =SCREEN_WIDTH/2.0;

            [button setFrame:CGRectMake(i*btnWidth, 0, btnWidth, TopScrollViewHeight)];
        }
    }
...
-(void)updateShadowView
{
    ...
    _shadowImageView.frame = CGRectMake(button.frame.origin.x, CGRectGetMaxY(button.frame) - 5,self.lineWidth,2);
...
-(void)setViewControllers:(NSMutableArray *)viewControllers
{
    ...
    for (int i = 0; i < [_viewControllers count]; i++) {
        ...
        button.titleLabel.font = FontSize(_ButtonFontSelectedSize);
...
-(void)updateUIWithSelectedIndex:(NSInteger)index byButton:(BOOL)byButton
{
    ...
    for (UIButton *button in _buttons) {
        if ([_buttons indexOfObject:button] == _selectedIndex) {
            button.titleLabel.font = [UIFont systemFontOfSize:_ButtonFontSelectedSize];
            button.selected = true;
        }else{
            button.titleLabel.font = FontSize(_ButtonFontSelectedSize);
            button.selected = false;
...
-(CGSize)buttonSizeOfTitle:(NSString*)title
{
    ...
    NSDictionary *attributes = @{ NSFontAttributeName : [UIFont systemFontOfSize:_ButtonFontSelectedSize], NSParagraphStyleAttributeName : style };
