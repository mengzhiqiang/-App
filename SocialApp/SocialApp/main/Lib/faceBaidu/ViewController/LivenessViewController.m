    ...
    [[IDLFaceLivenessManager sharedInstance] livenessStratrgyWithImage:image previewRect:self.previewRect detectRect:self.detectRect completionHandler:^(NSDictionary *images, LivenessRemindCode remindCode) {
        switch (remindCode) {
            case LivenessRemindCodeOK: {
                ...
                if (images[@"bestImage"] != nil && [(NSArray*)(images[@"bestImage"]) count] != 0) {
                    ...
                    if (bestImage && weakSelf.backImage) {
                        weakSelf.backImage(@[data]);
                    }
                }
...
-(void)loadImageOfID:(NSData*)IDImage{
    ...
    NSString *encodedImageStr = [IDImage base64EncodedStringWithOptions:NSDataBase64Encoding64CharacterLineLength];

    NSDictionary * diction = [NSDictionary dictionaryWithObjectsAndKeys:@"37082819880724367x",@"userIdentity",@"孟志强",@"userName", nil];
