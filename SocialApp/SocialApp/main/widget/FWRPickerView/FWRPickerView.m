...
-(void)SubmitPicker{
    ...
    if (_backPickerIndex) {
        _backPickerIndex(_beginString);
    }
    
    
    if (_backPickerInfo) {
        NSDictionary*diction ;
        if (_pickerType==pickerDataTypeEdu) {
            diction =@{@"edu":_beginString } ;
        } else if (_pickerType==pickerDataTypestatue) {
            diction =@{@"statue":_beginString } ;
...
- (IBAction)random:(id)sender {
    ...
    for (int i = 0; i < self.dataSouce.count; ++i) {
        // 第i组里面的所有数据
        NSUInteger count = [(NSArray*)self.dataSouce[i] count];
        // 生成随机数去选中
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component {
    return [(NSArray*)self.dataSouce[component] count];
}
//列显示的数据
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    switch (component) {
        case 0:
            self.beginString = [NSString stringWithFormat:@"%d",row+1];
            break;
        case 2:
...
-(void)setDateOfEdu{
    ...
    NSMutableArray * array = [NSMutableArray arrayWithObjects:@"大专以下",@"大专", @"本科", @"研究生", @"博士", nil];
...
-(void)setupPickerStyle:(pickerDataType)style{
    ...
    _pickerType = style ;
    switch (style) {
        case pickerDataTypestatue:
            self.titiLabel.text = @"学籍状态";
            [self setDateOfstatue];
            [_pickerView reloadAllComponents];
            [self.pickerView selectRow:0 inComponent:0 animated:YES];
            _beginString  = @"1";
            break;
        case pickerDataTypeEdu:
            self.titiLabel.text = @"学历";
            [self setDateOfEdu];
            [_pickerView reloadAllComponents];
            [self.pickerView selectRow:0 inComponent:0 animated:YES];
            _beginString  = @"1";
