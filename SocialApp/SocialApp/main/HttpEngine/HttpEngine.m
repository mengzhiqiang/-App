                        ...
                        failure:(HttpFailureBlock)failure
{
    ...
    param[@"userId"] = [CommonVariable getUserInfo].accountId;
    ...
    NSMutableURLRequest *request = [[AFHTTPRequestSerializer serializer] multipartFormRequestWithMethod:@"POST" URLString:url parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        ...
        if (params) {
            ...
            if (token) {
                ...
                NSData *dataImage= [NSJSONSerialization dataWithJSONObject:dic options:NSJSONWritingPrettyPrinted error:nil];
              [formData  appendPartWithHeaders:@{@"token":token} body:dataImage];
            }
        }
